...
| #   | 무엇이 깨지나                                                                           | 어디서                                                                                                                                                               | 부탁                                                                                                              |
| --- | --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| ①   | 아이가 보내는 것이 전부 403 — 「엄마 · 아빠한테 알리기」 · 「고마워요」 · 「같이 하자」 | `Family.java:184-189` 가 `fromProfileId` 를 **호출한 계정의 프로필**로만 받는다. 계정 없는 아이 프로필은 누구의 것도 아니다                                          | 부모 계정이 **계정 없는 자녀 프로필 이름으로** 보낼 수 있게(같은 기기를 아이가 빌려 쓴다)                         |
...
아이가 "같이 하자" 고 부른 것과 "다 했어요" 를 화면이 다르게 말해야 합니다.
칸이 없던 동안 스티커 없는 말은 전부 완료로 읽혀서, 같이 하자고 부른 것이 부모 종에
「운동을 마쳤어요」 로 떴습니다(아이가 하지 않은 일을 했다고 말함).

```
kind: "DONE" | "CALL" | "PRAISE"
```

프론트는 이미 `kind: "DONE"`(다 했어요 알리기) · `"CALL"`(같이 하자)을 실어 보냅니다.
받아서 저장하고, 알림을 지을 때 `CALL` 이면 아래 `KID_CALL` 로 지어 주세요.
...
| `KID_CALL`      | 부모     | 아이가 「같이 하자」 를 눌렀을 때  | 서준이 같이 하재요                |
