...
### 가족 목표 · 운동이 어땠는지 — 지금은 기기에만

| 무엇                                               | 지금                                                     | 부탁                                                                                 |
| -------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| 이번 주 가족 목표(분)                              | 부모 기기에 저장(`ff-prefs`) — 다른 기기에서는 기본 90분 | `GET · PUT /families/{familyId}/goal` `{ weeklyMinutes }`                            |
| 가족이 이번 주 같이 움직인 분                      | 사람마다 캘린더를 받아 앱이 더한다                       | 캘린더 응답에 `familyMinutes` 를 같이                                                |
