...
export function QueryProvider({ children }: { children: ReactNode }) {
  ...
  /*
    날이 바뀐 채로 돌아오면 받아 둔 것을 비우고 다시 받는다. 홈 화면 앱은 밤새 열려 있다가 아침에 다시 보이는데,
    창을 다시 볼 때 다시 부르지 않게 해 두어(위) 어제의 「오늘」 링 · 오늘 운동이 그대로 떠 있었다.
    다시 받기만 하면 값이 같을 때 화면이 다시 그려지지 않아, 그릴 때 오늘(today())을 읽는 칸이 어제에 머물렀다 —
    비우고 받으면 그 칸도 다시 그려진다. 화면을 통째로 다시 세우지는 않는다: 보내지 못한 칸 · 적던 측정이 날아간다
  */
  useEffect(() => {
    let day = today();
    const onShow = () => {
