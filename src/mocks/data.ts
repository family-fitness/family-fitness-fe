 ...
 * 매번 따로 만들지 않아도 되게 하려는 것이다.
 *
 * | 구성원 | 상태 | 화면에서 확인할 것 |
 * |---|---|---|
 * | 박지현 (41, 엄마) | 계정 연결 · 측정 있음 · MEASURE_TOO | 정상 카드 |
 * | 이상현 (42, 아빠) | 계정 없음 · 초대 대기 | 초대코드 발급 흐름 |
 * | 이하준 (9, 첫째) | 측정 있음 · 유연성 약점 | 약점 기반 추천 |
 * | 이서아 (3, 둘째) | measurable=false | 측정 버튼이 아예 안 뜨는지 |
 * | 이도윤 (6, 셋째) | 측정 없음 · headline=null | "첫 측정을 등록하면" 빈 상태 |
 */
import type {
  ActivityDay,
  ActivitySummary,
...
export const fitnessTests: Record<string, FitnessTest> = {
  [P.mom]: {
    ...
    items: [
      ...
      { item: "GRIP_STRENGTH", value: 26.4, percentile: 61, grade: 2 },
    ],
  ...
  [P.hajun]: {
    ...
    items: [
      ...
      { item: "SHUTTLE_RUN", value: 42, percentile: 63, grade: 2 },
    ],
...
/** 공단 유튜브 운동영상. 라벨(연령·소음·공간)로 추천 필터가 걸린다 */
export const videos: ExerciseVideo[] = [
  ...
  },
  {
    ...
    domains: ["CARDIO", "POWER"],
    minAge: 8,
    maxAge: 60,
    noiseLevel: "NORMAL",
    spaceRequirement: "OUTDOOR",
    favorited: false,
  },
  {
    id: "55555555-5555-4555-8555-555555555506",
    youtubeVideoId: "OPf0YbXqDm0",
    title: "앉아서 하는 어깨 · 허리 스트레칭",
    channelName: "국민체육진흥공단",
    durationSeconds: 396,
  ...
  proposalItems: [
    {
      ...
      video: videos[0],
      rationale:
        "하준이의 앉아윗몸앞으로굽히기가 또래 상위 82% 위치로 4개 항목 중 가장 낮습니다. 유연성은 짧게 자주 하는 편이 효과가 커서 매일 5분으로 잡았습니다.",
    ...
    },
    {
      ...
      title: "제자리멀리뛰기 강화 놀이",
      description: "잘하는 걸 더 잘하게. 주 2회 10분.",
      targetProfileIds: [P.hajun],
      video: videos[4],
      rationale:
        "하준이의 제자리멀리뛰기가 상위 19%로 강점입니다. 백분위 75 이상이면 약점 보완 대신 강점 강화로 전환합니다.",
      targetItem: "STANDING_LONG_JUMP",
...
function buildDays(seed: number): ActivityDay[] {
  const days: ActivityDay[] = [];
  const today = new Date("2026-09-08T00:00:00Z");
  for (let i = 69; i >= 0; i -= 1) {
    const d = new Date(today);
    d.setDate(d.getDate() - i);
    // 요일과 seed 로 그럴듯한 패턴을 만든다. 주말에 더 많이 움직인다
    const weekend = d.getDay() === 0 || d.getDay() === 6;
    const noise = (i * seed) % 7;
    const active = noise > 2 || weekend;
    days.push({
      date: d.toISOString().slice(0, 10),
      minutes: active ? 10 + ((i * seed) % 35) : 0,
      selfReportedSteps: active ? 3000 + ((i * seed * 137) % 9000) : 0,
      completedMissions: active && noise > 4 ? 1 : 0,
    });
  }
  return days;
}
...
export const cheers: Cheer[] = [
  ...
  },
  {
    ...
    fromProfileId: P.hajun,
    fromDisplayName: "이하준",
    toProfileId: P.mom,
    stickerCode: "MUSCLE",
    message: null,
    createdAt: "2026-09-07T20:02:00Z",
  },
];
 ...
 */
function buildPredictionPoints(): Prediction["points"] {
  ...
  for (let i = 0; i <= 10; i += 1) {
    ...
    points.push({
      ...
      item: "SIT_AND_REACH",
      ageAt: age,
      p10: Math.round(Math.max(1, base - 9 - i * 0.4) * 10) / 10,
    ...
    points.push({
      ...
      item: "SIT_AND_REACH",
      ageAt: age,
      p10: Math.round(Math.max(1, improved - 10 - i * 0.6) * 10) / 10,
...
export const coachMessages: CoachMessage[] = [
  ...
  },
  {
    ...
    createdAt: "2026-09-06T13:00:04Z",
  },
];
...
export const facilities: Facility[] = [
  ...
  },
  {
    ...
    name: "성북문화체육센터",
    category: "체력인증센터",
    address: "서울 성북구 삼선교로 16",
    distanceKm: 3.1,
    phone: "02-2241-4000",
    latitude: 37.5891,
    longitude: 127.0102,
    programs: ["국민체력100 체력측정", "청소년 체력교실"],
  },
];
