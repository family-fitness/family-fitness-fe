 ...
 *
 * 고정 JSON 을 돌려주지 않는다. 화면이 실제로 마주칠 **규칙**을 그대로 구현한다.
...
import { FITNESS_ITEM_LIST, gradeFromPercentile } from "@/lib/fitness-items";
...
const db = {
  ...
  recentVideoIds: [seed.videos[0].id, seed.videos[2].id],
  coachMessages: structuredClone(seed.coachMessages),
  cheers: structuredClone(seed.cheers),
...
const identity = [
  ...
  http.post(`${BASE}/profiles/claim`, async ({ request }) => {
    ...
    if (!/^\d{6}$/.test(code)) {
      return fail(400, "INVALID_CLAIM_CODE", "초대코드는 숫자 6자리예요.");
    }
  ...
  http.patch<PathParams>(
    ...
    async ({ params, request }) => {
      ...
      const member = db.fitnessMap.members.find((m) => m.profile.id === profile.id);
      if (member) member.profile = profile;
  ...
  http.post<PathParams>(
    ...
    async ({ params, request }) => {
      ...
      const items = measured.map((i) => {
        ...
        return { item: i.item, value: i.value, percentile, grade: gradeFromPercentile(percentile) };
      });
      ...
      const test: FitnessTest = {
        ...
        overallGrade: gradeFromPercentile(overall) as FitnessGrade,
      };
      ...
      if (member) {
        ...
        member.weakestItem = sorted[0]?.item ?? null;
        member.strongestItem = sorted[sorted.length - 1]?.item ?? null;
...
const coaching = [
  ...
  http.post(`${BASE}/families/:familyId/coach/runs`, async () => {
    // RAG 검색과 편성에 걸리는 시간. 스켈레톤이 실제로 보이게 하려고 넣었다
    await new Promise((r) => setTimeout(r, 1200));
    db.coachRun = structuredClone(seed.coachRun);
    db.coachRun.createdAt = new Date().toISOString();
...
const missions = [
  ...
  http.get(`${BASE}/families/:familyId/cheers`, () => HttpResponse.json(db.cheers)),
  http.post(`${BASE}/families/:familyId/cheers`, async ({ request }) => {
    const body = (await request.json()) as SendCheerRequest;
    const cheer = {
      id: uuid(),
      fromProfileId: body.fromProfileId,
      fromDisplayName: db.profiles.find((p) => p.id === body.fromProfileId)?.displayName ?? "",
...
const videos = [
  http.get(`${BASE}/profiles/:profileId/videos/recommend`, ({ params, request }) => {
    ...
    const result = db.videos.filter(
      (v) =>
        (!noise || v.noiseLevel === noise) &&
        (!space || v.spaceRequirement === space) &&
        // 연령 라벨로 거른다. 유아용 영상이 부모에게 뜨면 안 된다
        (!profile || (profile.age >= v.minAge && profile.age <= v.maxAge)),
    );
  ...
  http.get(`${BASE}/profiles/:profileId/videos/recent`, () =>
    HttpResponse.json(
      db.recentVideoIds
        ...
        .filter((v): v is (typeof db.videos)[number] => Boolean(v)),
    ),
  ),
