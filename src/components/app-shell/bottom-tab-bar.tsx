...
/**
 * 하단 탭바.
 * 활성 탭은 색만 바꾸지 않고 아이콘에 원형 배경을 칠한다.
...
export function BottomTabBar() {
  ...
  return (
    <nav aria-label="주요 화면" className="fixed inset-x-0 bottom-0 z-40 flex justify-center">
      <div className="border-line bg-paper/95 max-w-phone w-full border-t pb-[max(env(safe-area-inset-bottom),0.35rem)] backdrop-blur">
        <ul className="flex">
            ...
            return (
              <li key={href} className="flex-1">
                <Link
                  ...
                  className="press flex flex-col items-center gap-1 pt-2.5 pb-1"
                >
                  <span
                    className={cn(
                      "grid size-9 place-items-center rounded-full transition-colors",
                      active ? "bg-signal text-white" : "text-faint",
                    )}
                  >
                    <Icon className="size-[21px]" strokeWidth={active ? 2.4 : 2} aria-hidden />
                  </span>
                  <span
                    className={cn("text-[11px] font-bold", active ? "text-signal" : "text-faint")}
                  >
                    {label}
                  </span>
                </Link>
