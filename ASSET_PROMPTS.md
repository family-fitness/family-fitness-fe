...
> 이번 주문은 여섯 묶음 69장이고, 전부 **아래 공통 규칙 하나**로 뽑는다.
>
...
> 옛 1 · 2차 그림 48장은 지웠다. 파일이 들어오면 `npm run assets` 로 그 자리가 채워진다.
...
1024 원본을 그대로 넣고 `npm run assets` 를 돌리면 여백을 자르고 크기를 줄이고,
...
레벨이 오르면 열리는 몸 놀이(`/kid/play`). 놀이마다 첫 칸에 선다.
1장의 **3단계(잎새) 키움이**로 그려 주세요. 사람은 그리지 않는다.

| 파일               | 놀이        | 키움이가                                            |
| ------------------ | ----------- | --------------------------------------------------- |
| `play/play-dice`   | 운동 주사위 | 자기 몸만 한 파랑 주사위를 두 손으로 번쩍 들고 있다 |
| `play/play-freeze` | 얼음땡      | 한 발을 든 채 딱 멈춘 자세, 발밑에 작은 얼음 조각   |
| `play/play-follow` | 따라 해 봐  | 한 손을 번쩍 들고 다른 손으로 이쪽을 가리킨다       |
...
사람 모습 그림도 이번에는 없다 — 가족은 이름 첫 글자 동그라미로 가리킨다.
