...
> 이번 주문은 다섯 묶음 66장이고, 전부 **아래 공통 규칙 하나**로 뽑는다.
>
> **화면의 아이콘도 그림으로 바꾼다** (9/23 요청 — "이모트는 쓰지 말고 이미지로").
> 요인 · 메뉴 · 참여 방식 자리의 선 아이콘은 5장의 그림이 오면 그 자리에 바뀌어 선다.
> 뒤로 · › · 재생 · 멈춤 · 체크 같은 작은 조작 기호만 아이콘으로 남긴다.
>
> 그림이 오기 전에도 화면은 돈다 — 레벨 캐릭터는 코드로 그린 임시 그림이 있고,
> 나머지는 선 아이콘으로 대신 서 있다. 파일이 들어오면 그 자리에 바뀌어 선다.
...
부른다 — 목록을 안 고치면 파일을 넣어도 아이콘이 그대로 서 있다.
...
화면에서 선 아이콘이 서 있던 자리다. **작게(24~40px) 줄여도 무엇인지 읽혀야 한다.**
