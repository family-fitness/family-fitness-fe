...
| 운동 한 칸 끝       | `POST /missions/{id}/sessions/{position}/done`                      | 요청 `{ profileId, activeSeconds, startedAt, endedAt }` → `{ position, verifiedBy, missionProgress, missionCompleted, xpGained }` — **그 사람의 진행**. `xpGained` 는 `/progress` 가 실제로 는 만큼. 참여자 아님 403 `NOT_A_PARTICIPANT` · 절반도 안 함 422 `TOO_SHORT` · 동의 철회 403 `CONSENT_REQUIRED` | 운동하기                                             |
