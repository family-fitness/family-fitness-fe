...
> 2026-09-25 · 프론트 `feature/FE-104-audit` 기준 · 서버 계약은 `src/lib/api/schema.ts`(백엔드 `/v3/api-docs` 에서 생성, 29개 주소)와,
> 노션 백엔드 「API 명세서」(9/17)와 맞대 봤습니다(0장).
...
노션 「API 명세서」(백엔드 · 2026-09-17 · 백엔드 `develop` 기준, 엔드포인트 54행)와 이 문서를 한 줄씩 맞대 봤다.

### 이름만 다르다 — 백엔드 설계안 이름으로 맞추자

같은 일을 하는 주소를 양쪽이 다른 이름으로 적었다. 백엔드 설계안 이름으로 맞추면 프론트가 경로만 바꾼다.

| 무엇          | 프론트가 부르는 것(3장)                                                 | 백엔드 명세(설계안)                                                       | 맞출 것                                                                                                                           |
| ------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| 운동 한 칸 끝 | `POST /missions/{id}/sessions/{position}/done`                          | `POST /missions/{id}/sessions/{seq}/complete`                             | 백엔드 이름. `seq` = `position`(칸 번호). 요청 · 응답은 3장 모양으로                                                              |
| 쉬는 날 카드  | `GET · POST /families/{id}/rest-days` · `DELETE …/rest-days/{date}`     | `GET · POST /families/{id}/rest-cards` · `DELETE …/rest-cards/{restDate}` | 백엔드 이름. 다만 **부모만**(명세는 「로그인 필요」), 되돌리기는 **오늘도**(명세는 「그날 전까지」)                               |
| 운동 클립     | `GET /clips?factor=&phase=&quiet=&q=&list=&profileId=`                  | `GET /exercises` · `GET /videos/{videoId}/exercises`                      | 백엔드 이름(구간 = exercise). 3장의 필터 · 응답 칸이 필요하다                                                                     |
| 클립 찜       | `POST /clips/{clipId}/favorite`                                         | 영상 단위 찜만(`POST /videos/{videoId}/favorite` 구현됨)                  | **구간 단위 찜**이 필요 — 직접 짜기는 영상이 아니라 구간을 담는다                                                                 |
| 편성 조건     | `profileId · date · minutes · quiet · place · focusFactor · withParent` | v2 `constraints: noise · space · place · intensity · parentMode`          | `quiet` → `noise`, `withParent` → `parentMode`, `place` 같음. `profileId · date · minutes · focusFactor` 를 더 받아야 한다(1장 ②) |

### 이미 맞는 것

- `GET /families/{id}/cheers`(받은 칭찬) · `GET /profiles/{id}/fitness-tests`(측정 이력) · `GET /families/{id}/league` — 설계안에 같은 주소로 있다
- `mission_sessions` · `mission_session_completions`(참여자 한 명의 칸 하나 · `completed_at`) — 4장의 「끝낸 칸은 사람마다 · 끝낸 날을 서버가 든다」 와 같다.
  응답에는 `participants[].doneSessions: [seq…]` 로 실어 달라
- 승인은 제안을 복사만 하고 `mission_sessions` 도 같이 복사 · 코치 제안의 `sessions[]` 에 `phase` — 1장 ③ 과 같다
- 띠(`band`) 기준 75 · 25 — 목과 같다

### 백엔드 수정 요청 — 프론트 규칙대로 바꿔 달라

아래 넷은 설계안(9/17)과 프론트 규칙(9/23 · 9/25 회의)이 다르다. **프론트 규칙대로 백엔드를 바꿔 달라.** 프론트는 지금 모양 그대로 둔다.

1. **리그 — 한 달이 한 판 · 다섯 티어 · 해낸 날로 겨룬다**
   - 설계안: 주 단위(일요일 마감) · 여섯 티어(+마스터) · 방 20가족 · 달성률 = 「기운」 획득 ÷ 목표(지난 4주 중앙값 × 1.1)
   - 바꿀 것: 한 달이 한 판(1일에 위 `promote` 집은 한 티어 올리고 아래 `demote` 집은 내린다) · 브론즈 · 실버 · 골드 · 플래티넘 · 다이아(마스터 없음) ·
     새 가족은 브론즈
   - 달성률 = 잡힌 운동 날 중 해낸 날 ÷ 잡힌 운동 날, 아이들 평균. 쉬는 날은 뺀다. 체력 점수 · 기운 · 식구 수로 겨루지 않는다
     (운동 잘하는 집이 이기는 리그는 못하는 아이에게 벌이 되고, 식구가 많다고 유리하면 안 된다)
   - 셀 날이 없으면 `rate` · `rank` 는 `null`(0% · 꼴찌가 아니다). 가족 이름만 싣고 집 안에서 누가 더 했는지는 싣지 않는다
   - 응답은 3장 「가족 리그」 모양(`GET /families/{id}/league?month=YYYY-MM`). 승급 · 강등이 없는 방(명세의 8가족 미만)은
     `promote · demote` 를 0 으로 — 프론트는 「다음 달도 ○○」 로 읽는다
2. **레벨 · 경험치 · 업적 · 이어서 한 날 — 아이마다**
   - 설계안: 가족 단위 「기운」(`GET /families/{id}/points`)만 있고 아이별 레벨이 없다
   - 바꿀 것: `GET /profiles/{id}/progress`(3장) — `level · xp · levelFloorXp · nextLevelXp · streakDays · activeDays · achievements[] · recentXp[]`.
     **부모 프로필도** 준다
   - 프론트가 이걸로 그리는 것: 키움이가 레벨 단계대로 자라는 모습 · 섬의 나무(운동한 날마다 한 그루 — `activeDays`) · 「N일째 이어서」 ·
     레벨과 업적 화면 · 리그의 우리 가족 프로필(아이마다 키움이와 Lv · 가족이 얻은 업적)
   - 줄지 않는 값이다 — 경험치 · 레벨 · 운동한 날 · 받은 업적(7장). 이어서 한 날은 쉬는 날을 건너서 잇되 더하지 않는다
   - 「기운」 · 가족 약속(`/points` · `/promise`)은 프론트가 부르지 않는다(5장)
3. **알림함**
   - 설계안: 보낼지 판단(`GET /profiles/{id}/nudge`)만 있다
   - 바꿀 것: 보낸 알림을 남겨 두는 알림함 — `GET /notifications?profileId=` · `POST /notifications/read`(3장).
     `kind`: `KID_DONE`(아이가 다 했어요 → 부모) · `REMEASURE`(마지막 측정이 한 달 넘음) · `PRAISE`(스티커 → 아이) · `MISSION_READY`(오늘 운동 등록 → 아이) ·
     `ACHIEVEMENT`(업적 → 아이) · `KID_THANKS`(고마워요 → 부모)
   - nudge 의 판단(보낼지 · 언제 · 어떤 말투)은 서버 안에서 그대로 쓰고, 보낸 것을 알림함에 남겨 달라
4. **아이 화면의 신체 점수**
   - 설계안 문구 규칙: 아이 화면에서 `parent_scope`(백분위 · 점수 · 체중)를 읽지 않는다
   - 바꿀 것: `overallPercentile`(또래 백분위 — 아이 화면의 「신체 점수」) 하나는 아이 화면이 읽어도 되는 값으로.
     자기가 어디쯤인지 아는 건 아이 권리다(9/25). 프론트는 또래 평균 50 선과 같이 그리고, 등수 · 순위로 바꾸지 않는다
   - 등급 · 요인별 백분위 · 약한 항목 · 체중은 지금처럼 부모 화면만 읽는다
...
- 동의 없음 · 측정 대상 아님은 **422**(명세 공통 규칙 — 도메인 규칙 위반). 목 · 프론트 규칙을 422 로 고쳤다(화면은 코드로 가르므로 문구는 같다)
- 등급 기준 **1등급 ≥ 85 · 2등급 ≥ 65 · 3등급 ≥ 40 · 그 외 참가**(명세 ⑪). 목을 맞췄다 — 화면은 서버가 준 `grade` 를 그대로 쓴다
- **10년 후 예측은 걷었다**(명세 9/17 범위 밖). 「10년 위 연령대」 화면이 없어 `POST /profiles/{id}/predictions` 를 부르지 않는다
...
| ⑥   | 짜는 과정이 끝에 한꺼번에 뜬다                                         | `CoachRun.steps` 를 단계마다 저장 — `steps_json` 은 이미 있다(명세). 단계가 끝날 때마다 적어야 폴링 중에 하나씩 보인다                                  |
| ⑦   | 운동 칸을 끝내도 기록이 안 남는다                                      | `POST /missions/{id}/sessions/{position}/done`(아래 3장 · 명세 설계안 이름은 `…/sessions/{seq}/complete`) — 지금 서버의 `activity/timer` 는 칸을 모른다 |
...
| 운동 한 칸 끝       | `POST /missions/{id}/sessions/{position}/done`                      | 요청 `{ profileId, activeSeconds, startedAt, endedAt }` → `{ position, verifiedBy, missionProgress, missionCompleted, xpGained }` — **그 사람의 진행**. `xpGained` 는 `/progress` 가 실제로 는 만큼. 참여자 아님 403 `NOT_A_PARTICIPANT` · 절반도 안 함 422 `TOO_SHORT` · 동의 철회 422 `CONSENT_REQUIRED` | 운동하기                                             |
...
```
sessions: [
  { position: 1, phase: "WARMUP" | "MAIN" | "COOLDOWN", title, factor, minutes,   // minutes 는 1 이상
    completed?,                                        // doneSessions 가 오기 전에만 프론트가 본다
...
- **끝냈는지는 사람마다다.** 칸에 `completed` 하나를 두면 형제가 같은 운동을 받았을 때(직접 짜기에서 여럿을 고른다)
  ...
  보호자가 끝낸 칸은 그 보호자 것뿐이다. `doneSessions` 가 아직 없으면 프론트는 칸의 `completed` 로 물러선다(형제 구분은 못 한다)
...
- `verifiedBy` 는 서버가 아는 것만 — 걸음수 같은 자기 신고는 `SELF_REPORT` 이고 부모 확인(`…/participants/{profileId}/confirm`)이 남는다.
  ...
  확인할 것이 없으면(`needsGuardianCheck: false` — 타이머 · 영상으로 이미 확인됐거나 벌써 확인했다) 바꾸지 않고 200
- 직접 짠 운동은 **보낸 차례대로** 한다(`position`). 프론트가 준비 · 본 · 정리로 다시 줄 세우지 않는다
- 여러 날에 걸친 운동(`startDate` < `endDate`)은 **한 번 하는 운동**이다 — 기간 안 어느 날 해도 된다. 달력 · 리그 · 경험치는
  오늘과 칸을 끝낸 날에만 센다(기간의 날마다 잡힌 날로 세면 한 번 해낸 운동이 달성률을 끌어내린다). 끝내 한 칸도 안 했으면
  지난 마지막 날 하루로 센다(어느 날에도 안 세면 잡아 두기만 해도 달성률이 지켜진다). 칸을 끝낸 날을 서버가 든다
...
| `POST /profiles/{id}/predictions`                                           | 10년 후 예측 화면을 9/25 걷었다(명세도 9/17 범위 밖)                                                                                                |
...
| `PATCH /profiles/{id}/body`(설계안)                                         | 키 · 몸무게는 측정 등록에 같이 보낸다 — `latest` 응답에 실어 주면 된다(2장)                                                                         |
| `GET /families/{id}/missions/today` · `GET /missions/{id}`(설계안)          | 오늘 것은 목록에서 날짜로 고른다. 단건 조회는 있으면 새로고침이 가벼워진다                                                                          |
| `POST /missions/{id}/skip`(설계안)                                          | 쉬는 날은 가족 단위 카드(`rest-cards`)로 쓴다                                                                                                       |
| `GET /families/{id}/points` · `PATCH …/promise`(설계안)                     | 「기운」 · 가족 약속 화면이 없다 — 레벨 · 경험치는 아이마다(0장 수정 요청 2)                                                                        |
| `GET /profiles/{id}/nudge` · `GET /meta/versions`(설계안)                   | 판단은 서버 안에서 — 화면은 알림함을 읽는다(0장 수정 요청 3) · 버전은 화면에 없다                                                                   |
| `POST /missions/{id}/feedback`(설계안)                                      | 운동 끝 칸의 「어땠어요」(쉬웠어요 · 딱 좋아요 · 힘들었어요)를 보낼 수 있다 — `{ profileId, feel: "EASY" \| "GOOD" \| "HARD" }` 로 받아 주면 붙인다 |
...
`POST /profiles/{id}/fitness-tests` · `GET /coach/runs/{id}` · `POST /coach/runs/{id}/approve` ·
...
- 상태 코드는 명세 공통 규칙대로 — 403 권한 없음(대부분 보호자 아님) · 409 상태 충돌 · 422 도메인 규칙 위반(동의 없음 · 측정 대상 아님)
